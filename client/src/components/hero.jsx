...
export function Hero() {
    ...
    return (<section className="relative min-h-screen pt-20 md:pt-24 overflow-hidden bg-gradient-to-b from-secondary to-background">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button size="lg" className="bg-primary hover:bg-accent text-primary-foreground rounded-full px-8 py-6 text-lg font-semibold group">
                Shop Now
                <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform"/>
              </Button>
              <Button size="lg" variant="outline" className="rounded-full px-8 py-6 text-lg font-semibold border-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground">
                Learn More
              </Button>
            </div>
          ...
          <div className="relative animate-fade-in-up">
            <div className="relative w-full aspect-square max-w-lg mx-auto">
              ...
              <div className="absolute inset-8 rounded-full overflow-hidden shadow-2xl bg-muted">
                {B2_IMAGES.map((src, i) => (<img key={src} src={src} alt={`Organic product ${i + 1}`} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${i === currentIdx ? "opacity-100 z-10" : "opacity-0 z-0"}`}/>))}
              </div>
