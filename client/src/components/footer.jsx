...
export function Footer() {
    return (<footer className="bg-foreground text-white">
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto max-w-md">
              <div className="relative flex-1">
                ...
                <input type="email" placeholder="Enter your email" className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/20 rounded-full text-white placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-primary"/>
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        ].map((social) => (<a key={social.label} to="#" className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-primary transition-colors" aria-label={social.label}>
          ...
          <div>
            ...
            <ul className="space-y-4">
              ...
              {categories.map((category) => (<li key={category.id}>
                  <Link to={`/shop/${category.name.toLowerCase().replace(" ", "-")}`} className="text-white/70 hover:text-primary transition-colors">
                    {category.name}
                  </Link>
                </li>))}
            </ul>
