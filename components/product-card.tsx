...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group bg-card rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-2 flex flex-col border border-border">
      {/* Image Container */}
      <div className="relative h-56 bg-muted overflow-hidden transform-gpu">
        <Image
          src={product.image}
          alt={product.name}
          fill
      ...
      <div className="p-5 flex-grow flex flex-col">
        ...
        <h3 className="text-lg font-semibold text-foreground mt-1 mb-2 group-hover:text-primary transition-colors line-clamp-1">
          {product.name}
        </h3>
        ...
        <div className="mt-auto flex flex-col gap-3">
          ...
          <div className="pt-2 h-10">
            {qty === 0 ? (
              ...
              <div className="w-full h-full flex items-center justify-between bg-primary/10 rounded-full p-1 border border-primary/20">
                <Button
                  variant="ghost"
                  size="icon"
